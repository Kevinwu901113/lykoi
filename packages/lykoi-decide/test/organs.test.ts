...
test('完整清单：头 + 导语 + 绑定节 + 通道节 + 动作节逐字', () => {
  const input: OrganInventoryInput = {
    bindings: [
      { channel: 'telegram', role: 'owner_primary', display_name: '所有者', user_id: 'user_001' },
  ...
  const expected = [
    ...
    '- IM 收发(她的社交躯体): messenger.send_text, 其中每次都要 所有者 点头的: messenger.send_text',
    '- 给 所有者 的通知: notify.send',
...
test('四禁之 1（SA-161）：channel_key 不在输出面（行类型上就没有该字段）', () => {
  ...
  const rows = [{
    channel: 'telegram', role: 'owner_primary', display_name: '所有者', user_id: 'u',
...
test('E4-3：器官模板使用实例称呼，绑定的显示名称逐字保留', async () => {
  const { FIXTURE_PERSONA } = await import('./persona-fixture.ts')
  const text = renderOrganInventory({ ...EMPTY, persona: FIXTURE_PERSONA,
    bindings: [{ channel: 'telegram', role: 'owner_primary', display_name: '{owner}', user_id: 'x' }],
    knownActions: ['notify.send'],
  })
  assert.ok(text.includes('telegram: {owner}'))
  assert.ok(text.includes('给 Owner 的通知'))
})
