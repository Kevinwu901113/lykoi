...
test('同 turn r0 abort→r1 reply：终局只有一次；迟到工具回包零副作用', async () => {
  ...
  const terminals = h.events.filter(e => e.type === 'turn/terminal')
  ...
  h.calls[0]!.resolve({ content: envelope({ decision: { kind: 'tool_call', reason: '', tool: { name: 'research_read_text', arguments: { url: 'https://example.com' } } } }) })
  await new Promise<void>(resolve => setImmediate(resolve))
  assert.equal(dispatches, 0)
  assert.equal(h.events.filter(e => e.type === 'turn/terminal').length, 1)
...
test('最多两次 revision；第三条新输入正常排队，r2 不再被打断', async () => {
  ...
  assert.equal(h.events.filter(e => e.type === 'turn/terminal').length, 2)
