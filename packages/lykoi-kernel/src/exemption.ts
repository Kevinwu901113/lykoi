 ...
 * SK-47/48）。M2 时住在 lykoi-converse（接口位）；M3-W1 迁入 kernel —— 消费位
 * （approval.check 的最后一步）在这里成真，lykoi-converse 原路 re-export 保持
 * 既有 import 面不变（与活体 kernel/redaction.py re-export shared 同一手法）。
 *
 * 豁免只由 **Exemption 类型**携带，从不由 params / 文本携带（S-69）：字符串
 * "E1"、字典 {category:'E1'}、null 一律伪造不出来 —— 构造函数私有，实例只能
 * 出自三个工厂。消费位置是 approval.check 的**最后一步**（SK-15 第⑨步），
 ...
 *    M3-W3 的出站器官落地 —— 本模块类别与判定已承重（check 第⑨步）。
 *
