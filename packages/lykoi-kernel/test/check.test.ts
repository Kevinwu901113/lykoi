...
test('interactive Task stop needs no second permission; resume, denies and hard floors remain', () => {
  isolateKernelState()
  for (const command of ['pause', 'cancel']) assert.equal(check('task.control', 'interactive', { id: 'task-test', command }), 'allow')
  for (const command of ['resume', 'approve', undefined]) assert.equal(check('task.control', 'interactive', { id: 'task-test', command }), 'ask')
  assert.equal(check('task.control', 'autonomous', { command: 'cancel' }), 'deny')
  writeRules({ always_allow: [], always_deny: ['task.control'], ask: [] })
  assert.equal(check('task.control', 'interactive', { command: 'cancel' }), 'deny')
  writeRules({ always_allow: [], always_deny: [], ask: [] })
  _setPolicyCoreForTest({ hardDecision: () => 'ask', capabilityProfile: () => null })
  assert.equal(check('task.control', 'interactive', { command: 'cancel' }), 'ask')
})
