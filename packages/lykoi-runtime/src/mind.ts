...
{"records":[{"id":"已有 ID 或新的稳定名字","kind":"thought|preference|self|moment","topic":"原问题或偏好维度","understanding":"当前理解/进展","open":"这个原问题仍缺什么证据或还有什么没完成；全部解决才填 null","evidence":["来源引用"],"links":["task/skill ID"],"reconsiderAt":null,"basis":"explicit|inferred","scope":"适用情境"}],"acknowledge":["已理解事件的原始 ID"],"continue":false}
...
thought 是自己的问题，preference 是对用户的理解，self 是从经历形成的自我理解，moment 是特定关系在此刻的短期状态。四者不能混淆。self 不改写出生定义或执行权限；记录其证据与适用范围，可以被后续经历修订。moment 必须额外提供 expiresAt（未来的 ISO 时间），scope 写清交谈对象；到期只退出当前工作集，历史仍可检索。不要把一次情绪概括为永久关系，也不按情绪自动加减人格分数。已缓和时更新 expiresAt 为当前时间即可结束。只保存可续接的结论，不保存原始思维链。没有进展可不写。此通道取代旧 inner.thoughts/resolve。
...
export class MindStore implements CharacterMind {
  ...
  migrate(memoryPath: string) {
    ...
    try {
      if (memory.prepare("SELECT name FROM sqlite_master WHERE name='narrative_versions'").get()) {
        const row = memory.prepare("SELECT id, content, created_at FROM narrative_versions WHERE narrative_class IS NOT 'narrative_only' ORDER BY id DESC LIMIT 1").get()
        if (row) this.receive({ id: `legacy-narrative:${row.id}`, source: 'legacy-narrative', reference: `memory:narrative:${row.id}`,
          content: `历史自我叙述，作为可重新评估的资料；请结合经历决定是否形成或修订 self，不自动当作当前结论：\n${row.content}`, createdAt: String(row.created_at) })
      }
  ...
  view(query = '', limit = 20): MindView {
    ...
    const records = this.db.prepare('SELECT document FROM mind_records').all().map(r => JSON.parse(String(r.document)) as MindRecord)
      .filter(r => query ? JSON.stringify(r).toLocaleLowerCase().includes(needle) : r.status !== 'released' && (r.kind !== 'moment' || !r.expiresAt || r.expiresAt > now))
      .sort((a,b) => Number(related(b)) - Number(related(a)) || Number(b.open !== null || b.kind !== 'thought') - Number(a.open !== null || a.kind !== 'thought') || Number(!!b.reconsiderAt && b.reconsiderAt <= now) - Number(!!a.reconsiderAt && a.reconsiderAt <= now) || b.updatedAt.localeCompare(a.updatedAt))
    ...
    try {
      for (const record of patch.records ?? []) {
        if (!record || typeof record.id !== 'string' || !record.id.trim()
          || !['thought','preference','self','moment'].includes(record.kind)
        ...
        if (record.kind === 'moment' && (typeof record.expiresAt !== 'string' || !Number.isFinite(Date.parse(record.expiresAt)))) throw new MindPatchError('invalid_record', 'relationship moment requires expiresAt')
