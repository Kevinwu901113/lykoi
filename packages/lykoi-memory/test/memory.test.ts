...
// WO-MEM-DECAY-01：期望版本 16 → 17（017 迁移登记的新版本号）；WO-CONTINUATION-01：
// 17 → 18（018 加表）。判定逻辑逐字未动（仍是"恰等于期望值，否则拒开"），双向拒开
// 也照旧：低于（未施加迁移的旧库）与高于（比我新的 schema）都不许读。
test('打开即断言 mind_schema==18：不等则抛明确错误（新体不得读不认识的 schema）', () => {
  assert.throws(() => new ReadOnlyMemory(makeFixture(16)), (err: Error) => {
    assert.match(err.message, /mind_schema version 16 != expected 18/)
    return true
  })
  assert.throws(() => new ReadOnlyMemory(makeFixture(17)), /mind_schema version 17 != expected 18/)
  assert.throws(() => new ReadOnlyMemory(makeFixture(19)), /mind_schema version 19/)
})
...
test('devstate：只读打开成功即 mind_schema==18（断言在构造器内完成）', { skip: devstateSkip }, () => {
