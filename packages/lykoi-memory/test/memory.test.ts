...
test('打开即断言 mind_schema==19：不等则抛明确错误（新体不得读不认识的 schema）', () => {
  assert.throws(() => new ReadOnlyMemory(makeFixture(16)), (err: Error) => {
    assert.match(err.message, /mind_schema version 16 != expected 19/)
    return true
  })
  assert.throws(() => new ReadOnlyMemory(makeFixture(18)), /mind_schema version 18 != expected 19/)
  assert.throws(() => new ReadOnlyMemory(makeFixture(20)), /mind_schema version 20/)
})
...
test('devstate：只读打开成功即 mind_schema==19（断言在构造器内完成）', { skip: devstateSkip }, () => {
