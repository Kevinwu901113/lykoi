...
7. **同步完整性清单**：本工单会改动 `resources/`。必须同步更新 `guardian/manifest.sha256` 中所有本工单改动或新增的受保护 `.py` 条目（含正确路径与新哈希），不得遗漏；如排查后又改动 `cognition/mind/memory/shared/surface/resources` 六目录中的其他 `.py` 或 `guardian/` 下任何 `.py`，同样逐项登记/刷新。
...
1. 相关测试通过（列出文件与结果）。
2. **必须单独运行并报告** `pytest tests/test_p0_integrity.py`；同时反向核对本工单所有改动/新增的受保护 `.py` 均已进入 `guardian/manifest.sha256` 且哈希正确。
3. `git diff` 全文。
4. §4 的排查清单（查了哪些资源、各自结论）。
