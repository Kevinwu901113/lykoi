...
> **状态:已签发(2026-08-18 14:1x CST)。**E 步 usage 复读为**提前读**(Kevin
> 于 14:10 跑,窗口实际覆盖合并后 ~14h):main **零样本**
> (合并后无人对话)——≥70% 判据无法判定而非不达标,**并入本单影子期切换门**
> (影子 3 天内旧路径逐字节不变,读数自然积累;不达标不切)。对照组
> autonomous 37%(基线 30%,n=14,轻微上漂记观察);stable_prefix_rebuilt=0
> 与零对话自洽。签发裁量:唯一可得信号(对照组健康、零错误、completion/次稳定)
> 无恶化指向,影子形态本身即安全网。

你是执行 Agent,在 `~/lykoi-work-l1` 工作。
**分支 `wo/u3` 已由治理侧建好(尖 `2b8c477f` = 活体 main),直接 checkout,不要自己建分支。**
