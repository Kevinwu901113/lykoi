...
writeFileSync(process.env.LYKOI_APPROVAL_RULES, JSON.stringify({ always_allow: ['research_browser.read_text', 'workspace.read', 'workspace.list', labName], always_deny: [], ask: [] }))
...
const conversation = () => makeConversation({ wiredActions: runtime.actions, capabilities: () => runtime.capabilities().filter(c => check(c.name, 'interactive') !== 'deny'),
...
try {
  ...
  let calls = 0
  const stop = runtime.register({ organId: 'temporary-specimen', sideEffects: [], capabilities: [{ name: labName,
  ...
  record('PASS', { root, modelCalls, usage: ctx.budget.usage() })
} finally {
