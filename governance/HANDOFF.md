...
| 主治理 Agent | Kevin 指定的治理会话 | 写工单、已授权隔离实现、协调独立复核、跨机协调、文档维护 |
| 执行 Agent | 按当前任务安排的本地或远程会话 | 按工单在隔离工作副本里动手 |
...
2. **不利用组写位改活体检出** `~lykoi/projects/lykoi`——代码改动一律在经核验的隔离治理工作副本和工单分支进行（历史服务器路径为 `~/lykoi-work`）；
...
## 三、工单机制（核心工作流）
    ...
    Kevin 意图
    → 你写工单（order.md）
    → 主 Agent 直接实现或按任务安排执行 Agent 在隔离工单分支实现
    → 未参与实现的复核方验收（完整 diff + 适用检查 + 验收标准）
    → 报告 Kevin
    → Kevin 授权合并/部署

工单目录：治理账户 `~/wo/<WO-ID>/`（order.md / report.md / run.log），同步归档到本仓库 `governance/wo/<WO-ID>/`（外加你写的 `review.md`）。

### 执行方式

遵循 Kevin 本次指定的模型和执行方式；未指定时使用当前会话配置，不因历史模板切换模型或强制派发。自主执行、澄清、阻塞与完成规则见根 `CLAUDE.md`。历史调用示例已移至 [执行器配置存档](docs/archive/agent-dispatch-2026-08-07.md)。
...
小标题分组与主题不完全对应，故有此索引。2026-09-06 修订了报告、长任务、状态判断与结束纪律；事故事实保留，现行规则以根 `CLAUDE.md` 及对应条目的修订为准。
...
| 同上 | 43、45、46 | 仅文档 |
| 工单正文与结束纪律 | 1、3 | 已固化：根 `CLAUDE.md`「工单纪律」（报告文件与独立验收） |
...
| 同上 | 2、23、33b、44 | 现行规则见根 `CLAUDE.md`「自主执行与阻塞」「构建与测试」；旧 wrapper 的退出码不是验收结果 |
...
**事故关联索引**（保留历史编号；执行要求引用现行规则，不逐单复制事故补丁）：

- 22（`claude -p` 本身要脱管）→ 28（wrapper 自己也要脱管）→ 46（Mac 侧 ssh 管道同理）；
- 23（等待时提前结束会话）→ 29（等待之外还会中途断线丢工作）→ 33b（把测试丢后台再等）
...
1. **完整报告和证据落入工单文件，最终回复给结论和链接。** 历史 stdout 收集器出现过三次只收到摘要的事故；仅实际使用该收集器时指定 stdout 格式，不禁止保存文件。
2. **按验收条目检查必要证据，不用篇幅衡量完整性。** 报告列出结果、检查证据和未解决项；具体产物数量仅在任务确有要求时规定。
...
23. **等待测试时仍须跟进任务。** WO-P2-03A 曾两次在等待全量测试时提前结束会话且未提交。现行要求是跟进任务句柄、日志与退出状态到结束；不禁止等待，也不强制为长测试另拆工单或在测试前提交。
...
33b. **异步执行必须有结果回收。** 2026-08-12 的事故是在启动后台测试后直接交卷、会话退出导致测试丢失。后台或并行执行本身不受一概禁止；按根 `CLAUDE.md` 的长任务规则追踪并回收结果，共享写入和资源争用须串行。
...
43. **执行器状态须由任务身份和进程证据核验。** 2026-08-21 曾因截断进程列表漏看存活执行器，重复派发导致同树竞写。无 EXIT 行只表示状态未知；核对任务标识、PID/启动时间、进程列表、日志进展和退出记录。过滤须覆盖目标任务，不能以截断输出或单一无匹配结果认定死亡；确认没有存活写者后才可续跑。

44. **退出码和报告长度都不能代替验收。** 2026-08-22 的两次事故均在长测试进行中结束会话。按根 `CLAUDE.md` 区分实现、复核、合并和部署阶段，核对实际产物与适用检查。遇到真实外部阻塞可提交受阻报告，不把“已启动”或“稍后继续”算完成；相同提交、相同相关环境的有效基线证据可复用。
...
- **白皮书 v1.3 措辞稿已写（2026-09-05）**：`docs/whitepaper_v1.3_wording_draft_2026-09-05.md`。
  ...
  （"声明的因果入口必须有消费者"，37.7 增补，替代方案 37.9 供裁）、C-9 实例事实唯一出处（8 章新增 8.8，36 章"现状已满足"改口）。
  发现：37.3 已有"逐字整形"条款但只授予通讯集线器（条件立项），实现落在适配器与"零认知哑换能器"字面相撞——增补段把整形
  定性为传输并以"逐字"为判据。**待 Kevin 裁**：五条逐条、C-7 落点、8.8 是否 [PARTIAL]、终局三条是否合并、修订窗何时开
  （本稿建议五条齐了就开）。C-1/C-2/C-8 卡在对话读数，C-5 卡在 D2。
...
- **LANDING-M 已落（2026-09-03 23:49 CST，产线钉 main@91a47cf，一次通过）**：WO-FIX-TOOLFRAME-01 ——
  ...
  **已查实并立单 WO-OPS-BROWSER-PROXY-01（零代码，00:55 放行；实际 Kevin 00:42:43 已跑过一次成功——放行前落地，脚本备份/drop-in/记账齐全，后两次重跑被 §0 幂等拒绝）**：host.json `proxy:""` 直连不通（coingecko/kraken 10 s 000），
...
- **LANDING-J 已落（2026-09-03 16:11，产线钉 main@47fb05a，一次通过）**：WO-FIX-TOOLSTEP-01 ——
  ...
  toolstep 4/4、deepseek wire 7/7，downtime 6 秒，NRestarts 0。**已验（16:52）**：Kevin 第二条消息 24 秒内闭环——step 0 点
  research_read_text 读 weather.com.cn 2748 字 → step 1（关思考）2.4 s 出 reply 87 字 → telegram/sent；无回滚。
  **新问题**：DeepSeek json_object 空白输出——16:39 那条 step 1 两次采样均 58 token 纯空白（finish stop，
  有 charge）→ 有界重试耗尽 → silence；16:51 那条 step 0 也空过一次（重试成功）。今日 5 跳 3 跳出空白，与 wake
  not_json 同源。重试是**原样重发**（conversation.ts:910-930，同 messages 同 options，命中缓存原样复现），
  且 converse 的 retried/failed 事件未带 reasoning_len（D-2b 只做了 wake）。候选小单：重试时加引导语或换采样、
  retried/failed 事件补 reasoning_len。另记：百度搜索页对无头浏览器给 0 字，她自己换 weather.com.cn 成功。遗留（Kevin 裁）：step 0 是否
...
- Mac 侧有权限分类器会拦某些操作（自我提权类、写权限规则、动她的记忆库）。**停止被拒动作及其依赖步骤，不绕过限制**；继续其余已授权工作，集中报告被阻动作、拒绝原因及所需用户动作。
...
5. 将结果、验证与未解决项写入对应工单，提交并推送工作分支。执行报告须经未参与实现的复核方独立复核；未获得复核时标记“实现完成，待独立复核”，不阻止其余已授权的交付准备。
