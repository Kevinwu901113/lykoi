...
## 6 · 整批续接时的范围校正（2026-09-07）

实际扫描除原表六包，还发现 kernel 审批解释/建议回答、learn 的整合/规则建议提示、memory 的不兼容库错误含第一实例名称。本单将这些固定称呼改为“所有者”或中性错误文本，并更新对应 SHA；不改变审批算法、policy-core、工具名或存储格式。涉及包数以报告 diff 为准。kernel 的提示文字纳入本单可审阅交付，仍须治理复核后才可合入。

候选动作的 cost 与 note 均须渲染。模板替换仅作用于框架静态文字，不对用户消息、历史正文、器官绑定名称或经验引文执行全串替换；实例值中的模板符号也不递归解释。
