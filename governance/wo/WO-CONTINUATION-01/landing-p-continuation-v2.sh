#!/bin/bash
# LANDING-P v2 · 三单合批落地（WO-OUTCOME-01 + WO-OVERLAY-WAKE-01 + WO-CONTINUATION-01）—— 产线树 main@3c47c2e → bundle 里的 main
# v2：v1 的 EXPECT_OLD 误写为 O 之后的治理文档提交（从未落产线）；产线钉点是 LANDING-O 的 3c47c2e。
#   v1 在 §0 前验第一条 FATAL 退出，服务器零改动。v2 只改 EXPECT_OLD 与回滚注释里的旧 sha，其余逐字节相同。
#   3c47c2e 到 v1 所写旧点之间只动 governance/（HANDOFF、landing-o 稿、order），profile/deploy/依赖断言对 3c47c2e 同样成立。
...
# 用法（root）：
#   sha256sum /tmp/lykoi-landing-p.bundle /tmp/landing-p-continuation-v2.sh   # 与 Mac 上 shasum -a 256 的值逐字对
#   sudo bash /tmp/landing-p-continuation-v2.sh 2>&1 | tee /root/landing-p-v2-$(date +%Y%m%dT%H%M%S).log
set -euo pipefail
...
EXPECT_OLD=3c47c2e9e4ff5424b21fff0c28c59c0fbdcf432c
...
#   git checkout -f --detach 3c47c2e9e4ff5424b21fff0c28c59c0fbdcf432c
