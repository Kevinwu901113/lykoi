...
## 四b. 收尾核实（2026-08-09 23:27 执行 Agent 退场后补录）

- **EXIT=0（attempt=1）**，工作树干净，**已自行提交** `c308b792`——教训 23 的"不提交就退场"
  本单未复发。conftest 改动为新 percept_buffer 库加测试环境变量（必需且最小）。
- **基线对照终验**：在干净临时 worktree 检出起点 `94be1f2e`，与 `wo/p2-01` 尖端对同一批
  可疑文件（m3_r1a1_rollout / shadow / deepseek_v4_compat）各跑一遍，失败清单
  **逐条一致、diff 为空**——无任何新引入失败（governance-ops 已记录；worktree 已清理）。
  另：9 个权限位失败在干净 worktree 甚至不复现（umask 产物），进一步坐实环境伪影判定。
