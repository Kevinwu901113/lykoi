...
- 未接传输不再产生假成功；损坏持久化状态不再被清空或跳过。
- 模型可见数字使用原生 toFixed 精度；精确半点从 Python ties-to-even 改为原生舍入。未改数据库原始值和审批阈值。

验证：1199 项测试（1188 通过、11 项既有跳过）；最后修正的 Decide 断言单包 96/96 通过，其他包的全量结果沿用；typecheck 与 noUnusedLocals 通过。新增坏 JSON、持久化故障、损坏账本、未接传输和非法强度测试。历史源码注释清理单独比较去注释后的编译输出；64 个源码文件仅注释/空白变动，去注释编译输出一致；kernel 仅另删无消费者的 _DEFAULT_RULES，gate 权限策略未改。另用 noUnusedLocals 检查并清理无用 import、默认规则和测试常量。
