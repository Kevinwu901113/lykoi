...
代码与文档的最后一个改动提交是 `790a7ee`（R-2）；其后是本报告段自己的提交
（以及这一行的订正提交），分支 `wo/m5-organ-browser` 的实际 tip 以 `git log` 为准 ——
报告段没法在正文里写死自己的 sha，那是个先有鸡还是先有蛋的东西。
复核请按分支名取尖，本轮内容全部在 `1c249d6..` 之间。
