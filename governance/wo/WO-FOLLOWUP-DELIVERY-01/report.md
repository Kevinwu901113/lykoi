...
后续额度审计发现的四处问题已修复，详见 budget-review.md 与本文末尾最新验证；下方 64756 ms 是前一版历史样本，不代表最新严格验收通过。
...
## 后续额度修复的最终复核（2026-09-11）

四处修复及升级/回滚边界见 budget-review.md。全量 npm test：1234 项，1223 通过、0 失败、11 跳过；typecheck 与 diff check 通过。此前全量的一项失败是旧测试仍要求主动建议盖 E1；已改为不带豁免，并以实际建议调用方测试验证额度拦截。

真实模型再验保留两个新失败：live-shared-budget.log 到期后再次读取聊天而进入审批（这次审批成功送达）；随后明确分离模型 output 契约与内部 delivery 回执。最终 live-output-contract.log 在 **61980 ms** 经实际 BotApiTransport 向本地 HTTP 接收端发送了一条消息，Task delivery=sent、工具操作 0 次，但消息末尾多了一个中文句号，与要求的精确文本不符，严格验收失败。脚本在精确文本断言处退出，后续重复扫描和账本不变断言未执行，不能记为通过。

没有自动裁剪标点，没有重复抽样至通过。四处额度/来源缺陷已被调用方回归覆盖；固定文本逐字交付仍有模型行为缺口，PR 保持草稿。真实生产 Telegram 和旧生产任务均未修改，不能宣称生产已验收。
