...
| Regulation hard-prune / 饥饿例外 / 预算折扣 | 删除按 coherence/load 删候选、补回 explore 的例外及半额预算；删除无消费者 prefer_rest；原始状态仍可见 | 实际小时/通知/主动联系额度和当前注册能力；kernel 审批与派发门未改 |
...
| provider JSON workaround | 移到 lykoi-llm；单次 LLM call 最多三次请求、语法括号修复、取消检查、逐次 gate/charge；只重试明确 EMPTY_RESPONSE 或坏 JSON | 无工具重放；不修复截断字符串；审批外层仅重试 deadline/临时 HTTP，不重试协议恢复耗尽；合法 JSON 的业务语义由认知校验 |
