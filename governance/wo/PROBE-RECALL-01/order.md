...
history 实际列为 event_type，不是 type；按真实schema写统计SQL。d定义为第41轮查询向前看的深度，植入轮为41−d，故d≤8恰在最后8轮窗口内。6段×4深度×3装配×2重复=144次回忆请求，加48次摘要请求；两次重复各自生成摘要，避免把同一摘要冒充两次独立试验。

W8+M只是理想命中条件，不实现或测量真实检索命中率；所有方案为冻结合成装配，不宣称复现生产Conversation完整预算/缓存路径。usage包含供应商返回的token/cache字段，不据字符数估造token。自动名词/数字规则只出候选标签，否定与无关数字由人工复核。
